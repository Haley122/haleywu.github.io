...
The utilization of Raman scattering in temperature measurement stems from Boltzmann's law, which states that the probability of an atom being in an energy state is proportional to \\(exp((\varepsilon_i-\varepsilon_j)/kT)\\). Moreover, the ratio of atoms in energy states i and j is proportional to \\(exp(-(\epsilon<sub>i</sub>-\epsilon<sub>j</sub>)/kT))\\). Therefore, for two fixed energy levels i and j, at higher temperatures, a larger number of atoms will be in the higher energy level.
